...
class UserCell: UITableViewCell {
    var message : Message? {
        didSet {
            ...
            //cell.textLabel?.text=messages[indexPath.row].toUid
           detailTextLabel?.text=message?.text

        }
    }
    func setUserNameAndProfile(){
        
        let reciepientID : String?
        let currentUserId = FIRAuth.auth()?.currentUser?.uid
        if  currentUserId == message?.fromUid {
            reciepientID = message?.toUid
        }else{
            reciepientID = message?.fromUid
        }
        
        
        if let toId=reciepientID {
